...
namespace ItemM.Data.SqlMapper
{
    public static partial class SqlMapperExtensions
    {
        ...
        private static List<PropertyInfo> KeyPropertiesCache(Type type)
        {
            ...
            var keyProperties = allProperties.Where(p => p.GetCustomAttributes(true).Any(a => a is System.ComponentModel.DataAnnotations.KeyAttribute)).ToList();

            if (keyProperties.Count == 0)
            {
                var idProp = allProperties.FirstOrDefault(p => p.Name.ToLower() == "pkid");
                if (idProp != null)
                {
